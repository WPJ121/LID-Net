...
parser.add_argument('--data_dir', default='G:/', type=str, help='path to dataset')#D:/Dataset/RESIDE/RESIDE源/SOTS/
parser.add_argument('--save_dir', default='./saved_models/', type=str, help='path to models saving')
parser.add_argument('--result_dir', default='./results/', type=str, help='path to results saving')
parser.add_argument('--dataset', default='RESIDE-transportation-Test', type=str, help='dataset name')
...
if __name__ == '__main__':
	...
	network.cuda()
	saved_model_dir = os.path.join(args.save_dir, args.exp,  args.model+'.pth')    #/saved_models  +/outdoor  +/args.model+'.pth'
	...
	dataset_dir = os.path.join(args.data_dir, args.dataset)   #data/TEST/
	test_dataset = PairLoader(dataset_dir, 'test', 'test')    #data/TEST/+  test
	test_loader = DataLoader(test_dataset,
							 batch_size=1,
							 num_workers=args.num_workers,
							 pin_memory=True)

	result_dir = os.path.join(args.result_dir, args.dataset, args.model)     #/result +/TEST  +/model
